...
public class CircleMovement : MonoBehaviour
{
    ...
    void Update()
    {
        if (move == true)
        {
            ...
            if ((Math.Abs(transform.position.x - patrolPoints[currentPointIndex].position.x) <= 0.1) && (Math.Abs(transform.position.y - patrolPoints[currentPointIndex].position.y) <= 0.1) )
            {
